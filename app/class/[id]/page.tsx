    ...
    <div className="-mx-4 -mt-4 min-h-screen bg-bg-primary pb-12">
      ...
      <div className="px-4 space-y-4">
        ...
        <div className="flex gap-2">
          <ShareClassButton
            classId={cls.id}
            title={cls.title || 'Ninja H.E.R.O.S. Class'}
            ageGroup={cls.age_group}
            date={formatDate(cls.class_date)}
          />
          {cls.blocks.length > 0 && (
            <Link
        ...
        {cls.photos && cls.photos.length > 0 && cls.blocks.length === 0 && (
          <div className="card p-4">
            <p className="text-xs font-semibold text-text-dim uppercase tracking-wider mb-3">
              Photos
            </p>
            <div className="grid grid-cols-2 gap-2">
              {cls.photos.map((url: string, idx: number) => (
