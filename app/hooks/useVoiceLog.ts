...
export function useVoiceLog() {
  ...
  // Compute support lazily — never at module scope (SSR safe)
  ...
  function startRecording() {
    ...
    // iOS Safari may auto-stop after ~60s — handle gracefully
    recognition.onend = () => {
      setVoiceState((prev) => {
  ...
  async function parseTranscript(
    ...
    try {
      const res = await fetch('/api/parse-class', {
        method: 'POST',
      ...
      // Fallback: dump raw transcript so coach never loses content
      setVoiceState('error')
      setErrorMessage('AI parse failed — raw transcript saved to notes.')
      return {
        title: null,
        matched_curriculum_id: null,
        notes: `[Voice transcript — please review]\n${text}`,
      }
    }
  }
