...
*(1 `a7d52b4` · 2 `9fa8de1` · 3 `3a20a68` · 4 `e5a9fe5`+`f5a5c73` · 5 `d8b6431`+`caa3a81` · migration 017 APPLIED live + committed `6a2b5a7` — gate-A eyeball list: 8b copy frame-vs-C3 · "Structure it ✨" placement (unauthored) · blue vs orange "Save to library" · notes-doc arrival motion (unauthored) · delete-last-step behavior)*
...
6. ☑ **TypeAgeSheet** (`4df0483`+`15f9eb2`; hotfix 5.5 ↑-disc+keyboard `86fbe6f`+`b6a891a`) — frames `8g`, `8h`. Type pills (defaults text-only) + multi-select age chips (no ✓ marks) + "Done" text-only Inter 600 white top-right. "+ Add type" inline mini-form = name → Add type (no toggle; plannable defaults TRUE in data). Monogram auto-tile for CUSTOM types only. D2 spring-settle arrival. **Session checkpoint: auto-opens once per APP LAUNCH (first log); silent last-used chip after; chip tap reopens.**
7. ☑ **PhotoSheet** (`1e7ebc7`+`9d39d3f`) — frame `8i`. Tiles, drag reorder (first = cover, tag follows), ✕ per tile, "+" add, "⊟ Sort into separate stations" row (~13.5px text) visible at 2+ photos. Wire the whisper: once per capture, at the 3rd photo, auto-fades ~4s.

**Group C — multi-station + graceful → iPhone gate C**
8. ☑ **SortBins + stepper** (`34a05a8`+`2fd1a5f`) — frames `8j`, `13a-1`, `13a-2`, `13c`, `13b`. Drag photos into labeled bins (a wide photo may enter multiple bins; N=1 = normal flow, no bins). Rhythm A: one at a time, fully — land on Station 1's photos → develop → CTA "Save & next station →" → quiet segment tick (S1 stepper: ~22×4px segments + "Station N of N" Inter 600 ~11px #9fb0c8; no ✓ glyphs) → screen swaps to next → last station CTA = "Save to library" → ONE plural celebrate ("2 stations logged. Forever."). Forward-only; header Save = Save & next in stepper mode; bail risks only the current draft; type/ages inherit, editable per station.
9. ☑ **Graceful states** (`0d4127a`; save-failure gap closed — insert awaited, celebrate held, stepper no-advance) · **Critique tool** (`95a68bf`+`40cf289`: ✎ note mode on preview builds → dev_notes table) — frames `15a`–`15d`. Mic denied (typing door + 1-tap Settings link) · no speech → retry · AI fail → manual edit · offline → record-now. Never a dead end.
10. ☑ **Chunk 10 gate-B fix pass** (`e250b60`+`45f8181`): ①Curriculum rename ②drag glitch = CSS-columns vs dnd order mismatch → 2-col flex ③PhotoViewer (unauthored, + hero swipe wired) ④SortBins overlap <740px → flowed layout ⑤station-scoped chip mid-run ⑥NotesDoc scroll (overflow hidden + touchAction:none) + menu portaled

**Cutover (after gate C): ☑ DONE (`__CUTOVER_HASH__`).** Transitional mapping so new cards render in the OLD library UI (title/cover/skills map; steps+cues → description slot — `saveComponent.buildDescription`, verified: list/detail/edit route all read the mapped columns, no crash) · every create-flow entry now points at `/log` (TabNav FAB, library empty-state CTA, library choice sheet) · old create wizard retired (`git rm` of `library/log-component/page.tsx` + the 13 log-flow components used only by it + `log-flow.css` + the dead `dev-capture` harness; `atoms.tsx` KEPT — the edit route + PlanItemSheet still import `WaveformInline`; the EDIT route `library/log-component/[componentId]` left intact) · single-mode "+ photo" chip at exactly 1 photo (ledger fix). Build + tsc clean. **CUTOVER COMPLETE — merge-ready, awaiting River's final merge gate.**
...
1. iPhone device test after each group (A/B/C). 2. Migration 017 approval (chunk 5) — ✅ APPLIED. 3. Final merge.

## Gate-A feedback (River, 2026-07-09, iPhone) → hotfix chunk 5.5 ✅ BUILT
1. ☑ **"Structure it ✨" pill VETOED** (ledger §4 corrected): the door is the continuity disc — mic (idle, no note) → waveform+✓ (recording) → **↑ (a note exists, spoken or typed → tap = structure)**. Same 54px footprint, crossfade language. Killed the pill + the "Structuring…"/retry line moved into the dock label slot. Built in `IdleDock.tsx` (new `ArrowGlyph` ↑ face — UNAUTHORED, flagged for River — + a mic↔↑ reconciliation effect crossfading 280ms; disc dims/disables while structuring; role=status "Structuring…" and role=alert failure both in the label slot) + `Capture.tsx` (pill removed; `onStructure`/`structuring`/`developError` wired to IdleDock).
2. ☑ **Typing keyboard-lift bug**: pinned the dock FLUSH to the visualViewport's occluded bottom edge = `innerHeight − (vv.offsetTop + vv.height)`, then lift = `max(0, occluded − 46)` (the dock's resting bottom gap) → zero gap on ANY keyboard. rAF-throttled, transform-only, listeners cleaned up. `IdleDock.useKeyboardLift`. Verified in-preview: occluded 336 → translateY(−290px), restores to 0 on close.
Remaining eyeball items (unanswered at gate A so far): 8b copy · blue-vs-orange Save CTA · notes arrival motion · delete-last-step. **NEW for River's eyeball: the ↑ glyph (no frame authored it).**
